...
class dataset_h5(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print(os.path.join('./dataset/AVIRIS/train/hsi', self.fns[index]))
        if 'dataset/' in self.fns[index]:
            fn = os.path.join(self.root, self.fns[index])
        else:
            if os.path.isfile(os.path.join('./dataset/AVIRIS/train/hsi', self.fns[index])):
                fn = os.path.join('./dataset/AVIRIS/train/hsi', self.fns[index])
            elif os.path.isfile(os.path.join('./dataset/AVIRIS/test/hsi', self.fns[index])):
                fn = os.path.join('./dataset/AVIRIS/test/hsi', self.fns[index])
            else:
...
#         x[x<0]=0
        ...
        if self.mode=='Train':
            ...
            xx = []
            for k in range(0, x.shape[1], self.width):     # revise the x.shape[0 or 1] for classification data
                y = x[:, k:k+self.width, :]
                y = torch.from_numpy(y.copy())
                xx.append(y)
            x = torch.stack(xx)
