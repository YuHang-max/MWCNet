...
class CNNTRANs(nn.Module):  # CNN-Transformer SR Decoder
    ...
    def __init__(self, in_nc, out_nc, nf=64, nb=16, gc=32, up_scale=4):
    ...
    def forward(self, x):
        ...
        trunk = self.trunk_conv(self.CTMB_trunk(fea))  # 多尺度融合块 + 卷积 # trunk: torch.Size([1440, 64, 32, 1])
        fea = fea + trunk  # torch.Size([1440, 64, 32, 1])
        ...
        if self.up_scale == 4:
            fea = self.lrelu(self.upconv2(self.lrelu(self.upconv1(self.pixel_shuffle(fea))))) # fea1: torch.Size([1440, 128, 64, 2])
            fea = self.upconv4(self.lrelu(self.upconv3(self.pixel_shuffle(fea))))  # torch.Size([1440, 172, 128, 4])
